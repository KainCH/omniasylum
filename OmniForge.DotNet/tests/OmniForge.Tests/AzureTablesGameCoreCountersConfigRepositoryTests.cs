...
using OmniForge.Infrastructure.AzureTables.Repositories;
using Xunit;

namespace OmniForge.Tests
{
    public class AzureTablesGameCoreCountersConfigRepositoryTests
    {
        ...
        public AzureTablesGameCoreCountersConfigRepositoryTests()
        {
            var tableConfig = Options.Create(new AzureTableConfiguration());
            _mockServiceClient
                .Setup(x => x.GetTableClient("gamecorecounters"))
        ...
        {
            _mockTableClient
                .Setup(x => x.GetEntityAsync<GameCoreCountersConfigTableEntity>(
                    It.IsAny<string>(),
                    It.IsAny<string>(),
                    null,
        ...
        public async Task GetAsync_ShouldMapEntityToModel()
        {
            var entity = new GameCoreCountersConfigTableEntity
            {
                ...
                ScreamsEnabled = true,
                BitsEnabled = true,
                UpdatedAt = new DateTimeOffset(2026, 1, 24, 0, 0, 0, TimeSpan.Zero)
            };

            var response = Response.FromValue(entity, Mock.Of<Response>());
            _mockTableClient
                .Setup(x => x.GetEntityAsync<GameCoreCountersConfigTableEntity>(
                    GameCoreCountersConfigTableEntity.GetPartitionKey("u1"),
                    GameCoreCountersConfigTableEntity.GetRowKey("g1"),
                    null,
                    It.IsAny<CancellationToken>()))
        ...
        public async Task SaveAsync_ShouldUpsertEntity_AndDefaultUpdatedAt()
        {
            ...
            _mockTableClient
                .Setup(x => x.UpsertEntityAsync(It.IsAny<GameCoreCountersConfigTableEntity>(), TableUpdateMode.Replace, It.IsAny<CancellationToken>()))
                .Callback<GameCoreCountersConfigTableEntity, TableUpdateMode, CancellationToken>((e, _, __) => captured = e)
