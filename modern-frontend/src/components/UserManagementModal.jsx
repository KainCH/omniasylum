...
import { useUserData, useFormState, useToast, useLoading } from "../hooks";
import {
...
const getFeatureConfig = (featureKey) => {
  ...
  const configs = {
    chatCommands: {
      ...
      title: "Chat Commands",
      description:
        "Enable interactive chat commands for viewers to check counters and stats",
    },
    channelPoints: {
      icon: "🏆",
      title: "Channel Points",
      description:
        "Allow viewers to spend channel points to interact with stream counters",
    },
    autoClip: {
      icon: "🎬",
      title: "Auto Clip",
      description:
        "Automatically create Twitch clips when reaching milestone achievements",
    },
    streamOverlay: {
      icon: "📺",
      title: "Stream Overlay",
      description:
        "Display live counters and notifications directly on your stream",
    },
    discordWebhook: {
      icon: "🔔",
      title: "Discord Webhook",
      description:
        "Send automated notifications to your Discord server for milestones",
    },
    customCommands: {
      icon: "⚙️",
      title: "Custom Commands",
      description:
        "Create personalized chat commands with custom responses and actions",
    },
    analytics: {
      icon: "📊",
      title: "Analytics",
      description:
        "Track detailed statistics and generate insights about your stream",
    },
    webhooks: {
      icon: "🔗",
      title: "Webhooks",
      description:
        "Connect with external services and tools through custom webhooks",
    },
    bitsIntegration: {
      icon: "💎",
      title: "Bits Integration",
      description:
        "Enable Twitch Bits/Cheers integration for interactive stream features",
    },
    alertAnimations: {
      icon: "✨",
      title: "Alert Animations",
      description:
        "Add visual effects and animations to stream alerts and notifications",
    },
    streamAlerts: {
      icon: "🚨",
      title: "Stream Alerts",
      description:
        "Display customizable alerts for follows, subs, raids, and other Twitch events",
    },
    discordNotifications: {
      icon: "🔔",
      title: "Discord Notifications",
      description:
        "Send automated milestone and event notifications to your Discord server",
    },
  };
...
const UserManagementModal = ({
    ...
    try {
      await withLoading(async () => {
        ...
        if (userFeatures?.streamOverlay) {
          try {
            const overlayData = await userAPI.getOverlaySettings(
              user?.twitchUserId
            );
        ...
        if (userFeatures?.discordWebhook) {
          try {
            ...
            const discordData = await notificationAPI.getSettings(
              user?.twitchUserId
            );
            ...
            Object.assign(settings, {
              enableChannelNotifications:
                discordData?.enableChannelNotifications || false,
              deathMilestoneEnabled:
                discordData?.deathMilestoneEnabled || false,
              swearMilestoneEnabled:
                discordData?.swearMilestoneEnabled || false,
              deathThresholds:
                discordData?.deathThresholds || settings?.deathThresholds || "",
              swearThresholds:
                discordData?.swearThresholds || settings?.swearThresholds || "",
            });

            setNotificationSettings(settings);

            // Load webhook URL
            const webhookData = await notificationAPI.getWebhook(
              user?.twitchUserId
            );
  ...
  const saveOverlaySettings = async () => {
    ...
    try {
      await withLoading(async () => {
        await userAPI.updateOverlaySettings(
          user?.twitchUserId,
  ...
  const saveDiscordSettings = async () => {
    try {
      await withLoading(async () => {
        ...
        if (
          formState?.discordWebhook &&
          !formState.discordWebhook.startsWith(
        ...
        // Save webhook URL
        await notificationAPI.updateWebhook(user?.twitchUserId, {
          webhookUrl: formState?.discordWebhook || "",
        });
  ...
  const testDiscordWebhook = async () => {
    ...
    try {
      await withLoading(async () => {
        await notificationAPI.testWebhook(user?.twitchUserId, {
          webhookUrl: formState?.discordWebhook || "",
        });
      ...
      showMessage(
        `Failed to test webhook: ${error?.message || "Unknown error"}`,
        "error"
      );
    }
  };
                ...
                >
                  ...
                  <button
                    ...
                    disabled={
                      ...
                      !newUserData.twitchUserId
                    }
                    style={{
                      padding: "8px 16px",
                      background:
                        !newUserData.username || !newUserData.twitchUserId
                          ? "#666"
                          : "#00ff88",
                      color:
                        !newUserData.username || !newUserData.twitchUserId
            ...
            <div style={{ display: "grid", gap: "15px" }}>
              ...
              {users.length === 0 && (
                ...
                >
                  ...
                  <p>
                    Users will appear here once they authenticate with the
                    system.
                  </p>
                </div>
              )}
    ...
    <div className="user-management-modal-overlay" onClick={onClose}>
      ...
      >
        ...
        <div className="modal-body">
          ...
          <FormSection
            title="🎛️ Feature Management"
            collapsible
            defaultExpanded
          >
            <div className="professional-features-grid">
              {Object.entries(defaultFeatures || {}).map(
                ([feature, defaultValue]) => {
                  // Ensure feature is valid
                  if (
                    !feature ||
                    typeof feature !== "string" ||
                    feature === "templateStyle"
                  )
                    return null;

                  const isEnabled =
                    formState?.features?.[feature] ?? defaultValue;
                        ...
                        <div className="feature-header">
                          ...
                          <div className="feature-details">
                            <h4 className="feature-title">
                              {featureConfig?.title || "Unknown Feature"}
                            </h4>
              ...
              {formState?.overlaySettings && (
                <div className="overlay-settings">
                  <ToggleSwitch
                    ...
                    onChange={(checked) =>
                      handleOverlayChange("enabled", checked)
                    }
            ...
            <FormSection title="🔔 Discord Notifications" collapsible>
              <div className="discord-settings">
                ...
                <InputGroup
                  ...
                  onChange={(e) =>
                    updateField("discordWebhook", e.target.value)
                  }
                ...
                <div className="notification-types">
                  ...
                  <div className="discord-notification-info">
                    <p>
                      <strong>ℹ️ Discord Notifications:</strong> Automatically
                      enabled when webhook URL is configured above.
                    </p>
                  </div>
                </div>
                ...
                <div className="milestone-settings">
                  ...
                  <ToggleSwitch
                    id="death-milestones"
                    checked={
                      notificationSettings?.deathMilestoneEnabled || false
                    }
                    onChange={(checked) =>
                      updateNotificationSetting(
                        "deathMilestoneEnabled",
                  ...
                  <ToggleSwitch
                    id="swear-milestones"
                    checked={
                      notificationSettings?.swearMilestoneEnabled || false
                    }
                    onChange={(checked) =>
                      updateNotificationSetting(
                        "swearMilestoneEnabled",
